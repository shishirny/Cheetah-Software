 ...
 */
TEST(Stoch, StochModel1) {
  FloatingBaseModel<double> stoch = buildStoch<double>().buildModel();

  // masses
  EXPECT_TRUE(fpEqual(8.2520, stoch.totalNonRotorMass(), .0001));
  EXPECT_TRUE(fpEqual(0.66, stoch.totalRotorMass(), .0001));

  // check tree structure
  std::vector<int> parentRef{0, 0, 0,  0, 0,  0,  5, 6,  7,
                             5, 9, 10, 5, 12, 13, 5, 15, 16};
  EXPECT_TRUE(vectorEqual(parentRef, stoch.getParentVector()));
  ...
  for (size_t i = 0; i < 18; i++) {
    inertiaSum += stoch.getBodyInertiaVector()[i].getMatrix() +
                  stoch.getRotorInertiaVector()[i].getMatrix() * 0.25;
  }
 ...
 */
TEST(Stoch, StochModel2) {
  FloatingBaseModel<double> stoch = buildStoch<double>().buildModel();
  Mat6<double> XTotal = Mat6<double>::Identity();
  Mat6<double> XRotTotal = Mat6<double>::Identity();
  for (size_t i = 0; i < 18; i++) {
    XTotal = XTotal + stoch._Xtree[i];
    XRotTotal = XRotTotal + stoch._Xrot[i];
  }
 ...
 */
TEST(Stoch, simulatorDynamicsDoesntCrashStoch) {
  FloatingBaseModel<double> stoch = buildStoch<double>().buildModel();
  DynamicsSimulator<double> sim(stoch);
  DVec<double> tau(12);
 ...
 */
TEST(Stoch, simulatorDynamicsABANoExternalForceStoch) {
  FloatingBaseModel<double> stochModel =
      buildStoch<double>().buildModel();
  DynamicsSimulator<double> sim(stochModel);
 ...
 * Checks that quatD, pd, vd, and qdd match MATLAB
 */
TEST(Stoch, simulatorDynamicsWithExternalForceStoch) {
  FloatingBaseModel<double> stochModel =
      buildStoch<double>().buildModel();
  DynamicsSimulator<double> sim(stochModel, true);
 ...
 */
TEST(Stoch, simulatorFootPosVelStoch) {
  FloatingBaseModel<double> stochModel =
      buildStoch<double>().buildModel();
  DynamicsSimulator<double> sim(stochModel);
 ...
 */
TEST(Stoch, hipLocationConvention) {
  ...
  auto quadruped = buildStoch<double>();
...
TEST(Stoch, ContactPositionVelocity) {
  FloatingBaseModel<double> stochModel =
      buildStoch<double>().buildModel();
  DynamicsSimulator<double> sim(stochModel);
  ...
  // fwd kin is included in this
  stochModel.setState(x);
  stochModel.forwardKinematics();  // compute forward kinematics
...
TEST(Stoch, InertiaProperty) {
  ...
  stochModel.setState(x);
  stochModel.massMatrix();

  DMat<double> H = stochModel.getMassMatrix();
